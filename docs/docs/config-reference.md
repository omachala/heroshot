...
| ↳ `browser.bypassCSP`           | boolean                                      | `true`        | Bypass Content-Security-Policy restrictions. Enabled by default for reliable `evaluate` actions                                       |
| ↳ `browser.reducedMotion`       | `"reduce"` \| `"no-preference"`              | -             | Emulate prefers-reduced-motion. Use `"reduce"` to disable animations before capture                                                   |
